...
if len(argv) >= 1:
 runningPath = os.path.dirname(os.path.realpath(argv[0]))   # the directory of this .PY file
else:
 sys.exit(0)   # Nothing we can do here
...
with open( runningPath + "/log.txt", "a") as f9:
 ...
 if len(argv) < 2:
  f9.write( "ERROR: %d input arguments, expected at least 2. Terminating\r" % len(argv) )
  sys.exit(0)
 ...
 # Search through the parameters for "-s" and "-o" tags
 for x in argv[1:]:
  if (bNextIsOut):
   bNextIsOut = False
   theName = x  
 ...
 if (theName == ""):
  f9.write( "ERROR: no output name specified with \"-o\" or \"--out\" parameter. Terminating\r" )
  sys.exit(0)
